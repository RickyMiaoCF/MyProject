...
void AnimationModel::Load(const char* FileName)
{
	...
	for (int i = 0; i < m_AiScene->mNumTextures; i++)
	{
		...
		// テクスチャ読み込み
		// 【NOTE】目前只支持内藏式读取贴图（无法从外部读取）（因此当从Blender中导出模型时记得采用内藏的方式。）
	...
	float BlendRatio)
{
	...
	for (auto pair : m_Bone)
	{
		...
		aiVector3D pos;
		pos = pos1 * (1 - BlendRatio) + pos2 * BlendRatio;			//插值（線形補間）实现两个动画的混合
		aiQuaternion rot;
		aiQuaternion::Interpolate(rot, rot1, rot2, BlendRatio);		//球面線形補間（Quaternian専用）
